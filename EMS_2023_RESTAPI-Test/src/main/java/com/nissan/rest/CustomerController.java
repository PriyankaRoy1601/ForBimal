...
public class CustomerController {
	...
	public ResponseEntity<APIResponse> Deposit(@PathVariable long accno, @PathVariable double money,
	...
	public ResponseEntity<APIResponse> Deposit(@PathVariable long accno, @PathVariable double money,
			@PathVariable long pan, @RequestHeader(value = "authorization", defaultValue = "") String auth)
	...
	public ResponseEntity<APIResponse> Withdraw(@PathVariable long accno, @PathVariable double money,
			@RequestHeader(value = "authorization", defaultValue = "") String auth) throws AccessDeniedException {
		//jwtUtil.verifyC(auth);
	...
	public ResponseEntity<APIResponse> transferAmount(@PathVariable long accno1, @PathVariable long accno2,
			@PathVariable double amount, @RequestHeader(value = "authorization", defaultValue = "") String auth)
			throws AccessDeniedException {
		//jwtUtil.verifyC(auth);
		int flag = customerService.transfer(accno1, accno2, amount);
		if (flag == 1) {
			...
			apiResponse.setData("COULDN'T TRANSFER");
			apiResponse.setStatus(200);
			apiResponse.setError(500);
			return ResponseEntity.status(apiResponse.getStatus()).body(apiResponse);
		}
	}
